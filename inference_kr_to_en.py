...
from vits_inference_ddp import Inference
...
def main():
    ...
    parser.add_argument('--lang', type=str, default='english', help="Language to use, e.g., 'korean'")
    ...
    intervals = load_intervals_from_txt(txt_fpath)
    speech_intervals = find_speech_intervals(in_fpath, intervals, threshold=0.001, hop_length=256)

    with open(txt_fpath, 'w') as file:
        for i, (start, end, senten) in enumerate(speech_intervals, 1):
            file.write(f"{i:04d}\t{start:.2f}\t{end:.2f}\t{senten}\n")

    # 원본 korean 과 final_english 번역본 읽기 
    with open(txt_kr_fpath, 'r', encoding = 'utf-8') as line:
        lines = line.read().splitlines()
    with open(txt_fpath, 'r', encoding = 'utf-8') as fid:
    ...
    for txt,line in zip(txts,lines):
        try :
            idx, onset, offset, en_original = txt.split('\t')
            kr_idx, kr_onset, kr_offset, kr_original = line.split('\t')
        except Exception as e:
            print(e)
            continue
        word_count = len(en_original.split()) 
        ...
        new_length_scale, current_frames, spk_emb = inference.infer_origin(wav_seg, kr_original, sample_rate, 1, origin_frame)

        if word_count <=1 : 
            if new_length_scale > 2.0 :
                new_length_scale = 1.0
            wav_out = inference.infer(wav_seg, en_original, sample_rate, 0, spk_emb,new_length_scale)
            ...
            new_translate ,w_sum= inference.infer_t(wav_seg, en_original, kr_original, sample_rate, 0, new_length_scale, origin_frame,model, spk_emb)
            response = openai.Completion.create(
                                    engine=engine,
                                    prompt=f"Create 60 sentences that are the same length as this sentence and have the same meaning. Also, do not change the name or proper noun.: '{new_translate}'",
                                    max_tokens=1400,
                                    n=1,
                                    temperature=1,
                                    top_p=1
            ...
            kr_list = inference.infer_kr(wav_seg, kr_original, sample_rate, 1, new_length_scale, spk_emb)
            ...
            for sentence in sentences :
                if len(sentence) == 0 :
                    continue
                score,path =  inference.infer_vowel(wav_seg, sentence, sample_rate,0, new_length_scale, spk_emb, kr_list, rep, i, origin_frame)
                if score ==0 :
                    continue
            ...
            sorted_by_dtw = sorted(score_total, key=lambda x: x[1])[:5]
            print(sorted_by_dtw)
            ...
            wav_out = inference.infer_sl_final(wav_seg, final_sentence, sample_rate, 0, spk_emb,final_speed, silence_interval)
            final_audio = inference.infer_english(wav_seg, final_sentence, sample_rate, 0, spk_emb, new_length_scale)
    ...
    new_vocal_audio_resampled = np.clip(new_vocal_audio_resampled, -1.0, 1.0)
    bgm_wav = np.clip(bgm_wav, -1.0, 1.0)
