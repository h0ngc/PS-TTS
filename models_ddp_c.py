...
class SynthesizerTrn(nn.Module):
  ...
  def generate_kr_list(self,kr_x, kr_frame, kr_vowel):
    ...
    while i < len(kr_x_list):
        ...
        if value in kr_vowel:
            ...
            i += 1
    return kr_list
  def infer_korean(self, x, x_lengths, spk=None, lidx=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
    ...
    kr_list = self.generate_kr_list(kr_x,kr_frame,kr_vowel)
    return kr_list
  ...
  def infer_score(self, x, x_lengths, rep,kr_list, spk, lidx=None, noise_scale=1, length_scale=1, noise_scale_w=1.,origin_frame=1):
    # label_dict = { 43:'a',47:'e', 51:'i', 63:'u',109:'ɑ',110:'ɐ', 111:'ɒ',116:'ɔ',123:'ə',126:'ɛ', 127:'ɜ', 142:'ɪ',175:'ʊ', 178:'ʌ'}
    ...
    eg_frame = torch.round(w).squeeze()
    eg_frame_sum =w.sum().item()
    eg_list = self.generate_eg_list(eg_x, eg_frame, en_vowel, special_vowel)
